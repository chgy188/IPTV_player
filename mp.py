import sys
from PyQt5.QtWidgets import QTextEdit,QVBoxLayout,QProgressBar,QSizePolicy,QApplication, QMainWindow, QListWidget, QHBoxLayout, QWidget, QMenu, QAction, QMessageBox, QLabel, QFileDialog
...
class FullScreenPlayer(QMainWindow):
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        window.player.set_size(video_window.size().width(), video_window.size().height())
        self.close()
        window.show()
...
class IPTVPlayer(QMainWindow):
    def __init__(self):
        ...
        screensize = screen.size()
        self.resize(int(screensize.width() * 0.68), int(screensize.height() * 0.68))
    ...
    def dropEvent(self, event):
        ...
        if urls and urls[0].scheme() == 'file':
            ...
            if file_extension == '.m3u':
                ...
                self.play_program(self.image_label,self.current_media)
    ...
    def play_program(self,program_url,pos=0):
        ...
        # self.position = pos  # 初始化播放位置
        
        
        def run():           
            player_size=video_window.size()
    ...
    def swap_fullscreen(self,event=None):
        ...
        if self.thread and self.thread.isRunning():
            ...
            self.player.set_size(self.full_screen_player.size().width(), self.full_screen_player.size().height())
